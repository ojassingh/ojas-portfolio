  ...
  return (
    <div className="py-10 grid place-content-center">
      <nav className="">
          ...
          {navItems.map((item, i) => {
            return (
              <motion.li
                ...
                className="font-medium text-center text-lg hover:text-white"
                key={i}
              >
                <motion.a href={item.link} className="py-3 hover:text-white">
                  {item.name}
                </motion.a>
              </motion.li>
            );
        ...
        <h1 className="text-cyan-600 brightness-150 text-center text-xs">
          (navbars are overrated)
        </h1>
      </nav>
    </div>
  );
