...
const Intro = () => {
    return(
      <div id='home' className='h-screen py-20 lg:mt-20 md:mt-20 xs:mt-0 xs:px-4 xs:w-screen'>
        ...
        <motion.div  whileHover={{translateY: -7}}  className='ml-3 mt-10 lg:px-40 md:px-10'>
          <p className='mt-3 text-lg xs:text-sm text-blue-500 tracking-wide'>{"<div id='super-cool-description' >"}</p>
            <div className='mt-3 ml-6 border-l-2 border-gray-500/50'>
                <h3 className='ml-4 mt-2 lg:text-lg md:text-md xs:text-sm text-blue-500 tracking-wide'>
                    {/* {"<h3>"} <span className='text-2xl font-medium '>I love building impactful web applications.</span> {"</h3>"} */}
                    {"<h3>"} <span className='text-chalk text-2xl md:text-xl xs:text-sm font-normal '>your friendly neighbourhood full-stack developer.</span> {"</h3>"}
                </h3>

                <div id='description' className='text-chalk  ml-4'>
                  <p className='text-lg md:text-md xs:text-sm text-blue-500 tracking-wide'>
                      {"<p>"}
                  </p>  
                  <div className='lg:w-128 md:w-128 xs:w-full border-l-2 border-gray-500/50 ml-4'>
                    <p className='lg:text-lg md:text-md xs:text-sm ml-2 leading-flow'>My name is Ojas Singh, and I am a full-stack developer and student based in Toronto. I have a vested interest in deep learning, cooking and playing tennis. Fun fact: I make amazing butter chicken and I own a pair of Crocs with Lightning McQueen jibbitz.</p>
                  </div>
                  <p className='mt-2 text-blue-500 lg:text-lg md:text-md xs:text-sm tracking-wide'>
                      {"</p>"}
                  </p>  
                </div>
            </div>
          <p className='mt-2 lg:text-lg md:text-md xs:text-sm text-blue-500 tracking-wide'>{"</div>"}</p>
        </motion.div>
        <motion.div className='flex ml-3 m-10 lg:px-40 md:px-10'>
          <p className='flex-1 text-chalk lg:text-lg md:text-lg xs:text-sm'>Let's connect. Here are my socials: </p>
